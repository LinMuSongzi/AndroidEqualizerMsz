...
//    var showDialog: DialogFragment? = null
    ...
    override fun initRun(mAudioEffect: AudioEffect, owner: LifecycleOwner) {
        if (viewModel.environmentalReverbs.isEmpty()) {
            viewModel.environmentalReverbs.apply {
                add(
                    EnableViewModel.EnvironmentalReverbInfo(
                        "setDecayHFRatio",
                        maxProx = 2000,
                        startPox = 100,
                        plusSum = -100,
        ...
        EnvironmentalReverbDialog().show(
            (owner as FragmentActivity).supportFragmentManager,
            "EnvironmentalReverbLifecycle" + System.currentTimeMillis()
        )
